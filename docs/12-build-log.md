...
## Judgment call: no self-built Pinnacle scraper (for now)

Operator floated building our own Pinnacle scraper. Declined, on judgment: (1) it cuts against an existing plan decision — Doc 1 §1.3 / Doc 5 §8 explicitly reject sportsbook scraping as a ToS/legal risk; (2) Pinnacle sits behind Cloudflare + JS and would be an anti-bot arms race; (3) we already get Pinnacle *legitimately* via OddsPapi (an aggregator that owns that ToS boundary). The real constraint is the 250-call/mo free tier, and the right answers to that are strategic spend (active-window focus, already built) and the **swappable provider seam** — add another free aggregator tier (e.g. SharpAPI free DK/FD, a 3-day Pinnacle trial) or go paid ($99 SportsGameOdds / $399 SharpAPI) if the pilot shows signal — not scraping. Revisit only if a validated edge justifies it and a compliant path exists.
