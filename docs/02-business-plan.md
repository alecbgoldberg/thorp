...
| 3 months (end Phase 3 canary) | Realized edge per contract vs. modeled edge, ratio in **0.5x-1.5x** (tracking the model, not wildly off); fill-rate vs. adverse-selection decomposed and reported; fair-value model calibration (Brier score against actual outcomes) computed and compared against the Doc 1 benchmark range (Kalshi 78% / Polymarket 67% / PredictIt 93% in the reference study); directional hit-rate vs. Kalshi's own closing price ≥50% on a sample large enough for a binomial test to be informative (≥100 independent signals). **All of these are now concretely computable, not just named — see Doc 11 for the exact formulas (entry edge / calibration error / markout decomposition) and the pickoff-rate metric that operationalizes "adverse-selection decomposition."** |
